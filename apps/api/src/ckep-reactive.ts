...
async function projectJournalEvent(client:PoolClient,row:any){
  ...
  return (await client.query(`SELECT * FROM knowledge_events WHERE id=$1 AND node_id=$2 AND workspace_id=$3`,[id,row.node_id,row.workspace_id])).rows[0];
}
...
export async function dispatchCkepJournalEvent(client:PoolClient,input:{journalEventId:string;nodeId:string;workspaceId:string}){
  ...
  const existing=await client.query(`SELECT * FROM ckep_reactive_dispatches WHERE journal_event_id=$1`,[input.journalEventId]);
  ...
  await client.query(`INSERT INTO ckep_reactive_dispatches(journal_event_id,node_id,workspace_id,knowledge_event_id,status) VALUES($1,$2,$3,$4,'processing') ON CONFLICT(journal_event_id) DO UPDATE SET status='processing',error=NULL,started_at=now(),completed_at=NULL`,[input.journalEventId,input.nodeId,input.workspaceId,knowledgeEvent.id]);
  try{
    ...
    const row=await client.query(`UPDATE ckep_reactive_dispatches SET status='succeeded',impact_run_id=$2,impact_count=$3,watch_match_count=$4,completed_at=now() WHERE journal_event_id=$1 RETURNING *`,[input.journalEventId,impact.run_id,impact.impacted_count,watches.length]);return row.rows[0];
  }catch(error:any){await client.query(`UPDATE ckep_reactive_dispatches SET status='failed',error=$2,completed_at=now() WHERE journal_event_id=$1`,[input.journalEventId,String(error?.message??error)]);throw error;}
}
