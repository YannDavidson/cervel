...
export async function querySemanticViews(c: PoolClient, input: {
  ...
  const counts = await c.query(
    `SELECT v.mega_tab,
      ...
      WHERE v.corpus_id=$1
        AND (v.branch_visibility<>'sealed' OR EXISTS(
  ...
  return {
    ...
    canonical_cko_count: semanticViews.reduce((sum, item) => sum + item.canonical_cko_count, 0),
    membership_count: semanticViews.reduce((sum, item) => sum + item.membership_count, 0),
