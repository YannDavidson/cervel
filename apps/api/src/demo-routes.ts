...
export function registerDemoRoutes(app:FastifyInstance){
 ...
 app.get("/demo/app.css",async(_r,reply)=>reply.type("text/css; charset=utf-8").header("cache-control","no-cache, must-revalidate").send(demoCss));
 app.get("/demo/app.js",async(_r,reply)=>reply.type("application/javascript; charset=utf-8").header("cache-control","no-cache, must-revalidate").send(demoApp));
