...
export async function resolveRetrievalScope(
  ...
  const principal = await client.query(
    `SELECT attributes FROM principals WHERE id = $1 AND node_id = $2`,
    [input.principalId, input.nodeId]
  );
  ...
  if (!isAdmin) {
    const grants = await client.query(
      `SELECT pb.resource_type, pb.resource_id, p.document, p.effect_default, pb.priority
       ...
       WHERE pb.node_id = $1 AND (pb.principal_id = $2 OR pb.principal_id IS NULL)
    ...
    for (const row of grants.rows) {
      ...
      if (row.resource_type === "cko") {
        if (grantsRetrieve) allow.add(row.resource_id);
        if (deniesRetrieve) deny.add(row.resource_id);
        ...
        const rows = await client.query(
          `SELECT id FROM knowledge_objects WHERE node_id = $1 AND ($2::uuid IS NULL OR workspace_id = $2)`,
  ...
  const snapshot = {
    ...
    libraryIds,
    allowedCkoIds: allowedCkoIds ?? "admin-node-scope",
    allowedOperations: ["ai_discover","ai_read","ai_retrieve"]
  };
  ...
  return {
    ...
    libraryIds,
    allowedCkoIds,
    policySnapshotHash: hash(snapshot),
    allowedOperations: ["ai_discover","ai_read","ai_retrieve"]
  };
}

export function rankCandidate(lexical: number, semantic: number): number {
  return Math.max(0, Math.min(1, lexical * 0.4 + semantic * 0.6));
}
...
export async function hybridRetrieve(
  ...
  const rows = await client.query(
    `WITH candidates AS (
       ...
       WHERE f.node_id = $1
         ...
         AND (cardinality($7::uuid[]) = 0 OR EXISTS (
              SELECT 1 FROM library_memberships lm WHERE lm.cko_id = f.cko_id AND lm.library_id = ANY($7::uuid[])))
     )
     SELECT * FROM candidates
     WHERE lexical > 0 OR semantic > 0
     ORDER BY (lexical * 0.4 + semantic * 0.6) DESC
     LIMIT $8`,
    [
      scope.nodeId,
      query,
  ...
  return rows.rows.map((row) => {
    const lexical = Number(row.lexical ?? 0);
    const semantic = Math.max(0, Math.min(1, Number(row.semantic ?? 0)));
    return {
      ...
      lexical,
      semantic,
      final: rankCandidate(lexical, semantic),
      citationUri: `${canonicalCKURI(node.rows[0].slug, row.cko_id)}#frag/${row.fragment_id}`
    };
  });
}
