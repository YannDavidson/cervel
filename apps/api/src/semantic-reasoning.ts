...
import { sanitizeGeneratedAnswer } from "./answer-boundary";
...
async function runConfiguredAdapter(client: PoolClient, contextPackageId: string, principalId: string, answerId: string, contested: boolean) {
  ...
  const evidence = await client.query(`SELECT ce.citation, f.text_content FROM context_evidence ce JOIN fragments f ON f.id=ce.fragment_id WHERE ce.context_package_id=$1 ORDER BY ce.ordinal`, [contextPackageId]);
  ...
  const chain = [prepared.plan.selected, ...prepared.plan.fallbacks], started = Date.now();
  ...
  let output: any = null, adapterId = "", lastError = "", chosenProvider = "";
  ...
  output.text = sanitizeGeneratedAnswer(output.text);
  const model = await client.query(`SELECT * FROM models WHERE node_id=$1 AND provider=$2 AND model_name=$3 AND purpose='reasoning' LIMIT 1`, [ccp.rows[0].node_id, output.provider, output.model]);
  let modelId: string;
  if (model.rowCount === 1) modelId = model.rows[0].id; else { modelId = uuidv7(); await client.query(`INSERT INTO models(id,node_id,provider,model_name,model_version,purpose,metadata) VALUES ($1,$2,$3,$4,'external','reasoning',$5::jsonb)`, [modelId, ccp.rows[0].node_id, output.provider, output.model, JSON.stringify({ adapter: adapterId })]); }
  const modelRunId = uuidv7(), outputHash = sha256(output.text);
  await client.query(`INSERT INTO model_runs(id,node_id,model_id,principal_id,operation,parameters,input_hash,output_hash,started_at,completed_at,status) VALUES ($1,$2,$3,$4,'external_reason_over_context',$5::jsonb,$6,$7,now(),now(),'succeeded')`, [modelRunId, ccp.rows[0].node_id, modelId, principalId, JSON.stringify({ context_package_id: contextPackageId, adapter: adapterId, external_response_id: output.external_response_id ?? null }), sha256(JSON.stringify({ query: ccp.rows[0].query, contextPackageId })), outputHash]);
  ...
  await appendProvenanceEvent(client, { nodeId: ccp.rows[0].node_id, eventType: "external_reasoning_applied", actorType: "model", actorPrincipalId: principalId, inputs: [{ resourceType: "context_package", resourceId: contextPackageId }], outputs: [{ resourceType: "response", resourceId: answerId, sha256: outputHash }], parameters: { adapter: adapterId, provider: output.provider, model: output.model, modelRunId, disclosure_receipt_id: prepared.id } });
...
export async function executeSemanticKnowledgeReasoning(client: PoolClient, contextPackageId: string, principalId: string) {
  ...
  const safeBaseAnswer = sanitizeGeneratedAnswer(base.answer);
  if (safeBaseAnswer !== base.answer) await client.query(`UPDATE answers SET answer_text=$2 WHERE id=$1`, [base.id, safeBaseAnswer]);
  ...
  await client.query(`UPDATE answers SET uncertainty = uncertainty || $2::jsonb, trace_summary = trace_summary || $3::jsonb WHERE id=$1`, [base.id, JSON.stringify({ semantic_conflicts: semanticConflicts.length, confidence: confidence.final, contested, external_reasoning_failed: Boolean(external?.failed), external_reasoning_error: external?.failed ? external.error : null }), JSON.stringify({ semantic_claim_count: semanticClaims.length, graph_reasoning_run_id: graph.id, graph_confidence: graph.confidence })]);
  return { ...base, answer: successfulExternal?.text ?? safeBaseAnswer, model_run_id: successfulExternal?.model_run_id ?? base.model_run_id, semantic: { claims: semanticClaims, conflicts: semanticConflicts, graph, confidence }, adapter: successfulExternal ? { provider: successfulExternal.provider, model: successfulExternal.model } : { provider: "local", model: "cervel-trace-deterministic", fallback_from: external?.failed ? external.adapter : null }, intelligence: { receipt_id: external?.receipt_id ?? null, route: external?.route ?? null }, uncertainty: { ...base.uncertainty, contested, semantic_conflict_count: semanticConflicts.length, confidence: confidence.final, external_reasoning_failed: Boolean(external?.failed) } };
}
