...
export interface CkepEnvelope {
  ckep: typeof CKEP_VERSION;
  event: { id:CkepUri; type:CkepEventType };
  scope: { node:CkUri; workspace:CkUri };
  subject: CkepResourceRef;
  transition?: { previous?:CkepStateRef|null; current?:CkepStateRef|null };
  temporal: { observed_at:string; effective_at?:string|null };
  evidence?: { sources?:CkUri[]; artifact?:CkUri; artifact_version?:number; fragments?:CkUri[] };
  epistemics: { confidence:number; status?:string };
  impact?: CkepImpact[];
  provenance: CkepProvenance;
  causality?: CkepCausality;
  integrity: CkepIntegrity;
...
export function canonicalEventUri(authority:string,workspace:string,eventId:string):CkepUri {
  const clean=(v:string)=>encodeURIComponent(v.trim());
  if(!authority.trim()||!workspace.trim()||!eventId.trim())throw new Error("CKEP_ID_COMPONENT_REQUIRED");
  return `cke://${clean(authority)}/workspaces/${clean(workspace)}/events/${clean(eventId)}` as CkepUri;
}
...
export function computeCkepHash(event:Omit<CkepEnvelope,"integrity"> & {integrity:Omit<CkepIntegrity,"hash">|CkepIntegrity}):string {
  const copy=JSON.parse(JSON.stringify(event)); if(copy.integrity)delete copy.integrity.hash;
  return createHash("sha256").update(canonicalJson(copy)).digest("hex");
}
export function computeIdempotencyKey(input:{scope:CkepEnvelope["scope"];eventType:CkepEventType;subject:CkepResourceRef;observedAt:string;transition?:CkepEnvelope["transition"]}):string {
  return createHash("sha256").update(canonicalJson(input)).digest("hex");
}

export function validateCkep(input:unknown):{ok:true;event:CkepEnvelope}|{ok:false;errors:string[]} {
  const e=input as Partial<CkepEnvelope>,errors:string[]=[];
  if(!e||typeof e!=="object")return {ok:false,errors:["CKEP_OBJECT_REQUIRED"]};
  if(e.ckep!==CKEP_VERSION)errors.push("CKEP_VERSION_UNSUPPORTED");
  if(!e.event||!CKE_RE.test(String(e.event.id??"")))errors.push("CKEP_EVENT_ID_INVALID");
  if(!e.event||!EVENT_SET.has(String(e.event.type??"")))errors.push("CKEP_EVENT_TYPE_INVALID");
  ...
  if(!e.temporal||!iso(String(e.temporal.observed_at??"")))errors.push("CKEP_OBSERVED_AT_INVALID");
  if(e.temporal?.effective_at!=null&&!iso(String(e.temporal.effective_at)))errors.push("CKEP_EFFECTIVE_AT_INVALID");
  if(!e.epistemics||!confidence(Number(e.epistemics.confidence)))errors.push("CKEP_CONFIDENCE_INVALID");
  if(!e.provenance||!String(e.provenance.method??"").trim())errors.push("CKEP_PROVENANCE_METHOD_REQUIRED");
  if(!e.integrity||!Number.isSafeInteger(e.integrity.sequence)||Number(e.integrity.sequence)<1)errors.push("CKEP_SEQUENCE_INVALID");
  if(!e.integrity||!/^[a-f0-9]{64}$/i.test(String(e.integrity.idempotency_key??"")))errors.push("CKEP_IDEMPOTENCY_KEY_INVALID");
  if(!e.integrity||!/^[a-f0-9]{64}$/i.test(String(e.integrity.hash??"")))errors.push("CKEP_HASH_INVALID");
  if(e.integrity?.sequence&&e.integrity.sequence>1&&!e.integrity.previous_event)errors.push("CKEP_PREVIOUS_EVENT_REQUIRED");
  if(e.integrity?.previous_event&&!CKE_RE.test(String(e.integrity.previous_event)))errors.push("CKEP_PREVIOUS_EVENT_INVALID");
  for(const uri of e.causality?.caused_by??[])if(!CKE_RE.test(String(uri)))errors.push("CKEP_CAUSE_URI_INVALID");
  if(e.causality?.root_event&&!CKE_RE.test(String(e.causality.root_event)))errors.push("CKEP_ROOT_EVENT_INVALID");
  ...
  for(const uri of [...(e.evidence?.sources??[]),...(e.evidence?.fragments??[]),...(e.provenance?.derived_from??[])])if(!CK_RE.test(String(uri)))errors.push("CKEP_EVIDENCE_URI_INVALID");
  if(e.evidence?.artifact&&!CK_RE.test(String(e.evidence.artifact)))errors.push("CKEP_ARTIFACT_URI_INVALID");
  if(errors.length===0&&e.integrity&&computeCkepHash(e as CkepEnvelope)!==e.integrity.hash)errors.push("CKEP_HASH_MISMATCH");
  return errors.length?{ok:false,errors}:{ok:true,event:e as CkepEnvelope};
}
...
export function mapKnowledgeEventToCkep(input:{authority:string;workspaceId:string;nodeId:string;row:LegacyKnowledgeEventRow;impacts?:LegacyImpactRow[];sequence:number;previousEventId?:string|null;causedBy?:string[]}):CkepEnvelope {
  const type=LEGACY_TYPE_MAP[input.row.event_type]; if(!type)throw new Error(`CKEP_LEGACY_EVENT_TYPE_UNMAPPED:${input.row.event_type}`);
  const node=`ck://${input.authority}/nodes/${input.nodeId}` as CkUri,workspace=`ck://${input.authority}/workspaces/${input.workspaceId}` as CkUri;
  const subject=`ck://${input.authority}/${input.row.subject_type}/${input.row.subject_id}` as CkUri;
  const eventId=canonicalEventUri(input.authority,input.workspaceId,input.row.id);
  const observedAt=new Date(input.row.observed_at).toISOString();
  ...
  const idempotency_key=computeIdempotencyKey({scope:base.scope,eventType:type,subject:base.subject,observedAt,transition});
  const integrityNoHash={sequence:input.sequence,previous_event:input.previousEventId?canonicalEventUri(input.authority,input.workspaceId,input.previousEventId):null,idempotency_key};
  const envelope={...base,integrity:{...integrityNoHash,hash:""}} as CkepEnvelope; envelope.integrity.hash=computeCkepHash(envelope); return envelope;
}
