...
export function classifyForCorpora(input:ClassificationInput,definitions:readonly CorpusDefinition[]=BUILTIN_CORPORA,threshold=.3):CorpusCandidate[]{
 ...
 return definitions.flatMap(def=>{let score=0;const reasons:string[]=[];let coordinate:CorpusRule|undefined;for(const r of def.rules){const hits=r.terms.filter(term=>fields[r.field].includes(term.toLowerCase()));if(hits.length){score+=Math.min(r.weight,r.weight*hits.length);reasons.push(`${r.field}:${hits.join(",")}`);coordinate??=r;}}if(!coordinate||score<threshold)return[];return[{corpus_key:def.key,confidence:Math.min(.99,Number(score.toFixed(3))),mega_tab:coordinate.mega_tab,tab:coordinate.tab,subtab:coordinate.subtab??null,reasons}];}).sort((a,b)=>b.confidence-a.confidence||a.corpus_key.localeCompare(b.corpus_key));
}
