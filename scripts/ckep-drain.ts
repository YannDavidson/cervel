...
async function main(){
  ...
  const pending=await db.query(`SELECT j.id,j.node_id,j.workspace_id FROM ckep_event_journal j LEFT JOIN ckep_reactive_dispatches d ON d.journal_event_id=j.id WHERE d.journal_event_id IS NULL OR d.status='failed' ORDER BY j.published_at,j.sequence LIMIT $1`,[limit]);
